...
class Pi3AttentionAnalyzer:
    """Pi3 Decoder 内部Attention分析工具"""
    ...
    def register_hooks(self, model):
        """
        为模型的所有attention层注册hook
        适配Pi3的decoder结构
        """
        print("[Attention Analyzer] Registering hooks...")
        
        for name, module in model.named_modules():
            # 匹配FlashAttentionRope或BlockRope内的attention
    ...
    def _create_hook(self, name: str):
        ...
        def hook(module, input, output):
            # FlashAttentionRope通常返回 (attn_output, attn_weights)
            ...
            if attn_weights is not None and isinstance(attn_weights, torch.Tensor):
                # 只保存spatial dimensions的attention(跳过跨帧的超大attention)
                # 检查attention的seq_len维度,只保存相对小的(patch-level)
                seq_len = attn_weights.shape[-1]
                
                # 如果seq_len太大(>2048),可能是全局attention,跳过
                if seq_len <= 2048:
                    self.attn_maps.append((attn_weights.detach().cpu(), seq_len))
                    self.layer_names.append(name)
                else:
                    print(f"    (Skipped {name} with seq_len={seq_len})")
        
        return hook
    
    def extract_cross_frame_attention(self, attn_maps: List[torch.Tensor],
                                      num_frames: int,
                                      patch_size: int,
                                      H: int, W: int) -> Dict:
        """
        从attention maps中提取跨帧的self-attention关系
        处理不同shape的attention maps
        
        Args:
            attn_maps: list of (attn_weights, seq_len) tuples
            num_frames: 帧数 N
            patch_size: patch大小
            H, W: 图像分辨率
        
        Returns:
            Dict包含各种cross-frame attention信息
        """
        token_h = H // patch_size
        token_w = W // patch_size
        num_patches = token_h * token_w
        
        print(f"[Cross-Frame Analysis]")
        print(f"  Patches per frame: {num_patches}")
        print(f"  Image size: {H}x{W}, patch size: {patch_size}")
        
        # 分组attention maps by seq_len
        attn_by_seq_len = {}
        for attn_weights, seq_len in attn_maps:
            if seq_len not in attn_by_seq_len:
                attn_by_seq_len[seq_len] = []
            attn_by_seq_len[seq_len].append(attn_weights)
        
        print(f"  Found {len(attn_by_seq_len)} different attention types:")
        for seq_len, attn_list in attn_by_seq_len.items():
            print(f"    - seq_len={seq_len}: {len(attn_list)} layers")
        
        # 使用最多的attention类型(通常是spatial-only attention)
        selected_seq_len = max(attn_by_seq_len.keys(), key=lambda k: len(attn_by_seq_len[k]))
        selected_attns = attn_by_seq_len[selected_seq_len]
        
        print(f"  Using seq_len={selected_seq_len} attention ({len(selected_attns)} layers)")
        
        # Stack selected attention maps
        attn_stack = torch.stack(selected_attns, dim=0)  # [L, B, H_heads, T, T]
        B, heads = attn_stack.shape[1], attn_stack.shape[2]
        T = attn_stack.shape[3]
        
        print(f"  Stacked shape: {attn_stack.shape}")
        
        # 跨head求平均
        attn_avg = attn_stack.mean(dim=2)  # [L, B, T, T]
        
        # 计算期望的frames数
        expected_frames_from_seq = T // num_patches
        actual_frames = min(num_frames, expected_frames_from_seq)
        
        print(f"  Inferred frames from seq_len: {expected_frames_from_seq}")
        print(f"  Using frames: {actual_frames}")
        
        # 只处理patch部分,跳过register tokens
        patch_start_idx = 5  # Pi3的register tokens数量
        
        # 重塑为帧级别
        try:
            # [L, B, N, num_patches, N, num_patches]
            attn_reshaped = attn_avg.reshape(
                attn_avg.shape[0], B, actual_frames, num_patches, actual_frames, num_patches
            )
        except RuntimeError as e:
            print(f"[Warning] Cannot reshape attention: {e}")
            print(f"  Expected: {attn_avg.shape[0]} x {B} x {actual_frames} x {num_patches} x {actual_frames} x {num_patches}")
            print(f"  Got total tokens: {T}")
            return {'cross_frame_attn': {}, 'attn_stack': attn_avg, 'token_h': token_h, 'token_w': token_w, 'num_patches': num_patches}
        
        # 提取帧间对应关系
        cross_attn_dict = {}
        
        for frame_i in range(actual_frames):
            for frame_j in range(actual_frames):
                if frame_i != frame_j:
                    cross_attn = attn_reshaped[:, :, frame_i, :, frame_j, :]
                    key = f'frame{frame_i}_to_frame{frame_j}'
                    cross_attn_dict[key] = cross_attn
        
        print(f"  Extracted {len(cross_attn_dict)} cross-frame attention pairs")
        
        return {
            'cross_frame_attn': cross_attn_dict,
            'attn_stack': attn_avg,
            'token_h': token_h,
            'token_w': token_w,
    ...
    def compute_motion_attention_map(self, cross_attn_dict: Dict,
                                     num_frames: int,
                                     token_h: int, token_w: int,
                                     H: int, W: int) -> torch.Tensor:
        """
        基于attention的统计特性计算运动注意力图
        高方差 + 低均值 = 高动态性
        
        Args:
            cross_attn_dict: 跨帧attention字典
            num_frames: 帧数
            token_h, token_w: token空间分辨率
            H, W: 原始图像分辨率
        
        Returns:
            motion_map: [B, 1, H, W] 运动显著性图
        """
        # 获取任意一个attention来推导B
        sample_key = list(cross_attn_dict.keys())[0]
        sample_attn = cross_attn_dict[sample_key]  # [L, B, num_patches, num_patches]
        L, B = sample_attn.shape[0], sample_attn.shape[1]
        
        motion_map = torch.zeros(B, 1, token_h, token_w)
        ...
        for frame_idx in range(num_frames):
            ...
            for other_frame_idx in range(num_frames):
                if other_frame_idx != frame_idx:
                    key = f'frame{frame_idx}_to_frame{other_frame_idx}'
                    if key in cross_attn_dict:
                        attn = cross_attn_dict[key]  # [L, B, num_patches, num_patches]
                        attn_list.append(attn)
            
            if len(attn_list) == 0:
                continue
            
            # Stack所有帧对的注意力 [L, B, num_patches, num_patches, num_other_frames]
            attn_stack = torch.stack(attn_list, dim=-1)
            
            # 对目标帧求平均（跨其他帧）
            # 形状: [L, B, num_patches, num_patches]
            attn_mean_cross = attn_stack.mean(dim=-1)
            
            # 沿源token维度求统计
            # attn_mean_cross[:, :, i, :] = 源token i 对目标token的平均注意力
            
            # 统计特性（跨层、跨head）
            attn_mean = attn_mean_cross.mean(dim=(0, 1, 3))  # [num_patches] 均值
            attn_std = attn_mean_cross.std(dim=(0, 1, 3))    # [num_patches] 方差
            
            # 动态性指标: 高方差 + 低均值 = 高动态
            motion_score = attn_std * (1 - attn_mean)
            
            # 重塑回空间
            motion_score_2d = motion_score.view(1, 1, token_h, token_w)
            
            # 上采样到原始分辨率
            motion_score_up = F.interpolate(
                motion_score_2d,
                size=(H, W),
                mode='bilinear',
                align_corners=False
            ...
            # 累积（跨帧求平均）
            motion_map[:, :, :, :] += motion_score_up / num_frames
        
        # 归一化
        motion_min = motion_map.min()
        motion_max = motion_map.max()
        motion_map = (motion_map - motion_min) / (motion_max - motion_min + 1e-6)
        
        return motion_map
    
    def visualize_cross_attention_heatmap(self, cross_attn_dict: Dict,
                                          frame_idx: int,
                                          token_h: int, token_w: int,
                                          H: int, W: int,
                                          threshold: float = 0.5):
        ...
        for other_frame_idx in range(10):  # 最多支持10帧
            key = f'frame{frame_idx}_to_frame{other_frame_idx}'
            if key in cross_attn_dict:
                attn = cross_attn_dict[key]  # [L, B, num_patches, num_patches]
                
                # 跨层跨head取平均
                attn_mean = attn.mean(dim=0)[0]  # [num_patches, num_patches]
                
                attn_list.append(attn_mean)
                frame_labels.append(other_frame_idx)
        
        # 可视化前4个
        for idx, (ax, attn, target_frame) in enumerate(zip(axes.flat, attn_list[:4], frame_labels[:4])):
            # 对目标token求平均（源token的平均注意力）
            attn_avg = attn.mean(dim=1)  # [num_patches]
            
            # 重塑回空间
            attn_2d = attn_avg.view(token_h, token_w).numpy()
            
            # 上采样
            attn_up = cv2.resize(attn_2d, (W, H), interpolation=cv2.INTER_LINEAR)
            
            im = ax.imshow(attn_up, cmap='hot')
            ax.set_title(f'Frame {frame_idx} → Frame {target_frame}', fontweight='bold')
            ax.axis('off')
            plt.colorbar(im, ax=ax)
        
        plt.tight_layout()
        plt.savefig(self.save_dir / f'cross_attn_frame{frame_idx}.png', dpi=150)
        plt.close()
    
    def visualize_motion_attention_map(self, motion_map: torch.Tensor,
                                       num_frames: int):
        """
        可视化最终的运动注意力图
        """
        ...
        fig, axes = plt.subplots(1, min(B, 4), figsize=(5*min(B, 4), 5))
        if B == 1:
            axes = [axes]
        
        for b in range(min(B, 4)):
            motion = motion_map[b, 0].numpy()
            
            im = axes[b].imshow(motion, cmap='RdYlBu_r')
            axes[b].set_title(f'Batch {b} - Motion Attention', fontweight='bold')
            axes[b].axis('off')
            plt.colorbar(im, ax=axes[b])
        
        plt.suptitle(f'Motion Attention Maps ({num_frames} frames)', 
                     fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(self.save_dir / 'motion_attention_map.png', dpi=150)
        plt.close()
        
        print(f"[Visualization] Saved to {self.save_dir / 'motion_attention_map.png'}")
    
    def visualize_attention_statistics(self, cross_attn_dict: Dict):
        """
        可视化注意力的统计特性（均值/方差/分布）
        """
        stats = {
            'means': [],
            'stds': [],
        ...
        for key, attn in cross_attn_dict.items():
            # [L, B, num_patches, num_patches]
            attn_mean = attn.mean().item()
            attn_std = attn.std().item()
            
            stats['means'].append(attn_mean)
            stats['stds'].append(attn_std)
            stats['labels'].append(key)
        
        # 可视化
        fig, axes = plt.subplots(2, 1, figsize=(14, 8))
        
        x_pos = np.arange(len(stats['labels']))
        
        # 均值
        axes[0].bar(x_pos, stats['means'], alpha=0.7, color='skyblue')
        axes[0].set_ylabel('Mean Attention', fontsize=12)
        axes[0].set_title('Cross-Frame Attention Mean Distribution', fontweight='bold')
        axes[0].set_xticks(x_pos)
        axes[0].set_xticklabels(stats['labels'], rotation=45, ha='right', fontsize=9)
        axes[0].grid(axis='y', alpha=0.3)
        
        # 方差
        axes[1].bar(x_pos, stats['stds'], alpha=0.7, color='lightcoral')
        axes[1].set_ylabel('Std Attention', fontsize=12)
        axes[1].set_title('Cross-Frame Attention Std Distribution', fontweight='bold')
        axes[1].set_xticks(x_pos)
        axes[1].set_xticklabels(stats['labels'], rotation=45, ha='right', fontsize=9)
        axes[1].grid(axis='y', alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(self.save_dir / 'attention_statistics.png', dpi=150)
        plt.close()
        
        print(f"[Visualization] Saved to {self.save_dir / 'attention_statistics.png'}")
    
    def analyze(self, model, video_tensor: torch.Tensor, 
                num_frames: int, image_size: Tuple[int, int],
                patch_size: int = 14) -> Dict:
        """
        完整的分析流程
        
        Args:
            model: Pi3MotionSeg模型
            video_tensor: [B, N, C, H, W] 视频数据
            num_frames: 帧数
            image_size: (H, W)
            patch_size: patch大小
        """
        print("\n" + "="*60)
        print("Pi3 Decoder Attention Analysis")
        print("="*60)
        ...
        # 前向传播
        print("\n[Forward Pass] Processing video...")
        with torch.no_grad():
            _ = model(video_tensor)
        
        print(f"✓ Collected {len(self.attn_maps)} attention maps from {len(set(self.layer_names))} layers")
        
        # 提取跨帧attention
        print("\n[Cross-Frame Extraction] Analyzing cross-frame attention...")
        result = self.extract_cross_frame_attention(
            self.attn_maps,
            num_frames,
            patch_size,
        ...
        # 检查是否成功提取
        if not result['cross_frame_attn']:
            print("[Error] Failed to extract cross-frame attention")
            return {
                'motion_map': torch.zeros(1, 1, *image_size),
                'cross_attn': {},
                'attn_maps': self.attn_maps,
                'token_h': result['token_h'],
                'token_w': result['token_w']
            }
        
        # 计算运动attention map
        print("\n[Motion Computation] Computing motion attention map...")
        motion_map = self.compute_motion_attention_map(
            result['cross_frame_attn'],
            num_frames,
            result['token_h'],
            result['token_w'],
            image_size[0], image_size[1]
        ...
        # 跨帧热力图
        for frame_idx in range(result.get('actual_frames', num_frames)):
            self.visualize_cross_attention_heatmap(
                result['cross_frame_attn'],
                frame_idx,
                result['token_h'], result['token_w'],
                image_size[0], image_size[1]
            )
        
        # 运动图
        self.visualize_motion_attention_map(motion_map, num_frames)
        
        # 统计分布
        self.visualize_attention_statistics(result['cross_frame_attn'])
        ...
        return {
            ...
            'cross_attn': result['cross_frame_attn'],
            'attn_maps': self.attn_maps,
            'token_h': result['token_h'],
            'token_w': result['token_w']
        }
...
def run_pi3_attention_analysis(model, video_tensor: torch.Tensor,
                               num_frames: int, 
    ...
    Args:
        model: Pi3MotionSeg实例
        video_tensor: [B, N, C, H, W]
        num_frames: 帧数 N
        image_size: 图像分辨率
    ...
    返回:
        motion_map: [B, 1, H, W] 运动显著性图
    """
    analyzer = Pi3AttentionAnalyzer(save_dir)
    result = analyzer.analyze(model, video_tensor, num_frames, image_size, patch_size)
    
    return result['motion_map'], analyzer