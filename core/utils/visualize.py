...
def read_imgs_from_path(img_dir):
    img_paths = sorted(glob(os.path.join(img_dir, "*.png"))) + sorted(
        glob(os.path.join(img_dir, "*.jpg")) + sorted(glob(os.path.join(img_dir, "*.jpeg")))
    )
    img_list = []
...
class Visualizer:
    ...
    def visualize(
        ...
        if segm_mask is not None:
            ...
            # coords[:, 0] = torch.clamp(coords[:, 0], 0, segm_mask.shape[0] - 1)
            # coords[:, 1] = torch.clamp(coords[:, 1], 0, segm_mask.shape[1] - 1)
