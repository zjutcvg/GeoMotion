...
class Transformer(Module):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6,
                 ...
                 bias: bool = True, device=None, dtype=None, time_att=True) -> None:
        ...
        if custom_encoder is not None:
            ...
            encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout,
                                                    ...
                                                    bias,time_att=time_att, **factory_kwargs)
...
class TransformerEncoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            # time-attn: n,t,c
            x_time, src_key_padding_mask_t = x.permute(1,0,2), src_key_padding_mask.permute(1,0)
            x = x + self._sa_block(self.norm3(x_time), src_mask, src_key_padding_mask_t, is_causal).permute(1,0,2)          
            ...
            if self.time_att:
                x_time, src_key_padding_mask_t = x.permute(1,0,2), src_key_padding_mask.permute(1,0)
                x = self.norm3(x + self._sa_block(x_time, src_mask, src_key_padding_mask_t, is_causal).permute(1,0,2))            
...
class TransformerDecoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            x_time, tgt_key_padding_mask_t = x.permute(1,0,2), tgt_key_padding_mask.permute(1,0)
            x = x + self._sa_block(self.norm4(x_time), tgt_mask, tgt_key_padding_mask_t, tgt_is_causal).permute(1,0,2)
            ...
            if self.time_att:
                x_time, tgt_key_padding_mask_t = x.permute(1,0,2), tgt_key_padding_mask.permute(1,0)
                x = self.norm4(x + self._sa_block(x_time, tgt_mask, tgt_key_padding_mask_t, tgt_is_causal).permute(1,0,2))            
            x = self.norm2(x + self._mha_block(x, memory, memory_mask, memory_key_padding_mask, memory_is_causal))
            x = self.norm3(x + self._ff_block(x))

        return x

    # self-attention block: [t, n, c], space-attention
    def _sa_block(self, x: Tensor,
                  attn_mask: Optional[Tensor], key_padding_mask: Optional[Tensor], is_causal: bool = False) -> Tensor:
        x = self.self_attn(x, x, x,
                           attn_mask=attn_mask,
