...
def get_pipeline(model_name: str):
    # Prefer local checkpoint directory (user-provided) to avoid remote downloads
    # The local checkpoints should be present at preproc/checkpoints/
    try:
        pipe = pipeline(
            task="depth-estimation",
            model="/data0/hexiankang/code/SegAnyMo/preproc/checkpoints/",
            device=DEVICE,
        )
        print(f"{model_name} model loaded from local checkpoints.")
        return pipe
    except Exception:
        # Fallback to HF model id if local checkpoints are unavailable
        pipe = pipeline(task="depth-estimation", model=models[model_name], device=DEVICE)
        print(f"{model_name} model loaded from HF model id: {models[model_name]}")
        return pipe
