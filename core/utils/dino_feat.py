...
class ViTExtractor:
    ...
    def create_model(model_type: str) -> nn.Module:
        if "dinov2" in model_type:
            ...
            # 指定我们已经确认存在的 DINOv2 本地仓库的完整路径。
            local_dinov2_path = '/home/amax/.cache/torch/hub/facebookresearch_dinov2_main'

            # 从这个本地路径加载模型定义，source='local' 强制它只使用本地目录。
            model = torch.hub.load(local_dinov2_path, model_type, source='local')
...
if __name__=='__main__':
    ...
    if not img_paths_all:
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print(f"!!! WARNING: No images found in directory: {args.image_dir}")
        print(f"!!! This directory will be skipped.")
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        exit()  # 或者使用 continue，但 exit() 更明确地终止这个子进程
