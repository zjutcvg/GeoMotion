...
class HOI_dataset(Dataset):
    def __init__(self, data_dir, split, transform=None,
        ...
        self.sample_list = [
            item for item in sorted(os.listdir(img_dir_root))
            if os.path.isdir(os.path.join(img_dir_root, item))
        ]
        ...
        for seq_name in tqdm(self.sample_list, desc="loading data"):
            # Load image list.
            img_dir = osp.join(self.data_dir, "images",seq_name)
        ...
        max_num = num # 9216
        random_num = [1024, 1536, 2048, 4096, 5000, 6000]

        random_num = [x for x in random_num if x <= max_num]
    ...
    def __getitem__(self, idx):        
        ...
        for path in mask_paths:
            ...
            if mask_tensor.dim() == 3 and mask_tensor.size(0) == 3:
                mask_tensor = (mask_tensor > 0).any(dim=0).float()
        ...
        # Load DINO feature
        if self.load_dino:
            dino_dir = img_dir.replace("images", "dinos2")
            dino_path = osp.join(dino_dir,f'{frame_names[q_t]}.npy')
...
class MotionSegmentationDataset(Dataset):
    """
    ...
    def __init__(self, data_dir, split, transform=None, 
        ...
        """
        Args:
            data_dir: Root directory containing 'images' and 'dynamic_masks' folders
        ...
        # Load initial sample list from image directories
        img_dir_root = osp.join(self.data_dir, "images")
        self.sample_list = [
            item for item in sorted(os.listdir(img_dir_root))
            if os.path.isdir(os.path.join(img_dir_root, item))
        ...
        for seq_name in tqdm(self.sample_list, desc=f"Validating hoi4d data for '{split}' split"):
            img_dir = osp.join(self.data_dir, "images", seq_name)
            mask_dir = osp.join(self.data_dir, "dynamic_masks", seq_name)
            
            # 1. Check if mask directory exists
        ...
        print(f"Loaded {len(self.sample_list)} valid sequences for  HOI4D '{split}' split")
...
def test_motion_seg_dataset():
    ...
    """
    cfg = MotionSegConfig()
    cfg.data_dir = "/data0/hexiankang/code/SegAnyMo/data/HOI4D_clean"  # Update this path
