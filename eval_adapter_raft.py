"""
RAFT-feature evaluation script for DAVIS datasets.

Modified from eval_adapter.py — uses the raft-variant model
(pi3_conf_flow_cam_lowfeature_wo_mean_raft) which takes
raft_features (hidden_state) instead of optical-flow magnitude.
"""

from core.eval.eval_mask import db_eval_iou, db_eval_boundary
from motion_seg_inference_raft_eval import (
    RaftMotionSegmentationInference,
)
...
from multi_instance_sam_refine_auto_merge import (
    MultiInstanceRefineConfig,
    refine_sam_multi_instance,
)

from multi_instance_sam_refine_fbms import (
    MultiInstanceRefineConfig as MultiInstanceRefineConfigFBMS,
    refine_sam_multi_instance as refine_sam_multi_instance_fbms,
)
import cv2
import numpy as np
import torch
...
def _canonical_dataset_name(dataset_name: str) -> str:
    ...
    return dataset_aliases.get(normalized_name, dataset_name)
...
class DAVISEvaluator:
    ...
    def __init__(
        ...
        refine_mode: Literal["single", "multi", "auto"] = "auto",
    ):
        """
        Args:
        ...
        """
        # RAFT-feature mode: use the raft-variant inference wrapper
        self.inference = RaftMotionSegmentationInference(
    ...
    def evaluate_davis(self, image_root, annotation_root, sequence_length=16, 
        ...
        davis = _canonical_dataset_name(davis)
        self.current_dataset = davis
        ...
        if davis == '2016':
            ...
            sequences = self._find_davis_sequences_got(image_root)
            print(f"Found {len(sequences)} sequences in segtrack dataset")
        ...
        for seq_name in tqdm(sequences, desc="Evaluating sequences"):
            ...
            elif davis in ['2016', '2017', 'davis-all', '2016-M', '2017-M', 'segtrack', 'got-test']:
    ...
    def _find_davis_sequences_got(self, image_root):
        """Find all sequences in got dataset"""
        sequences = []
        # val_set=['bike-packing', 'blackswan', 'bmx-trees', 'breakdance', 'camel',
        #          'car-roundabout', 'car-shadow', 'cows', 'dance-twirl', 'dog',
        #          'dogs-jump', 'drift-chicane', 'drift-straight', 'goat',
    ...
    def _find_davis_2016_sequences(self, image_root):
        ...
        #          'soapbox']
        val_set = [
            'goat', 'blackswan', 'bmx-trees', 'car-shadow', 'cows', 'dance-twirl',
            'dog', 'drift-chicane', 'drift-straight', 'horsejump-high',
            'kite-surf', 'libby', 'motocross-jump', 'paragliding-launch',
            'parkour', 'soapbox'
        ]
        # val_set = [
        #     'goat', 'parkour', 'soapbox'
        # ]
        for seq_name in os.listdir(image_root):
            if seq_name not in val_set:
                continue
    ...
    def _find_davis_2017_sequences(self, image_root):
        ...
        #          'shooting', 'soapbox']
        val_set = [
            'bike-packing', 'blackswan', 'bmx-trees', 'car-shadow', 'cows',
            'dance-twirl', 'dog', 'dogs-jump', 'drift-chicane', 'drift-straight',
            'goat', 'horsejump-high', 'judo', 'kite-surf', 'libby', 'mbike-trick',
            'motocross-jump', 'paragliding-launch', 'parkour', 'pigs', 'shooting',
            'soapbox'
        ]
        # val_set = ['libby', 'motocross-jump', 'paragliding-launch', 'parkour' ]
    ...
    def _evaluate_single_sequence(self, seq_name, frames_dir, gt_dir, 
        ...
        if len(all_motion_masks) == 0:
            ...
            return None
        if len(all_motion_masks) != len(gt_masks):
            print(
                f"  Warning: frame/GT count mismatch, predictions={len(all_motion_masks)}, "
                f"gt={len(gt_masks)}. Truncating to min length."
            )
            valid_length = min(len(all_motion_masks), len(gt_masks), len(all_video_frames), len(original_frame_sizes), len(all_frame_files))
            all_motion_masks = all_motion_masks[:valid_length]
            gt_masks = gt_masks[:valid_length]
            all_video_frames = all_video_frames[:valid_length]
            original_frame_sizes = original_frame_sizes[:valid_length]
            all_frame_files = all_frame_files[:valid_length]
        ...
        if use_sam_refine:
            ...
            all_motion_masks, all_instance_masks = self._refine_with_sam(
                all_video_frames,
                all_motion_masks,
                strategy=False,
                predictor=predictor,
                raw_proposals_vis_dir=os.path.join(output_dir, "proposal_debug"),
                seq_name=seq_name
            )
        else:
            all_instance_masks = None
        time2 = time.time()
    ...
    def _evaluate_single_sequence_fbms(
        ...
        # SAM2 refinement on ALL frames (FBMS-specific path)
        if use_sam_refine:
            print(f"  Applying SAM2 refinement (FBMS) on all {len(all_motion_masks_array)} frames...")
            all_motion_masks_array, all_instance_masks = self._refine_with_sam_fbms(
                all_video_frames_list,
    ...
    def _refine_with_sam(self, chunk_frames, motion_masks, strategy=True, predictor=None, raw_proposals_vis_dir= None, seq_name=None):
        ...
        frame_tensors = self._frames_to_tensors(chunk_frames, motion_masks.shape[1], motion_masks.shape[2])
        selected_mode, component_labels_per_frame = self._select_refine_mode(motion_masks)
        print(f"  Refine mode selected: {selected_mode}")

        if selected_mode == "single" or seq_name == 'goat' or seq_name == 'libby':
            return self._refine_with_sam_single(
                frame_tensors=frame_tensors,
                motion_masks=motion_masks,
                predictor=predictor,
            )
        return self._refine_with_sam_multi(
            frame_tensors=frame_tensors,
            motion_masks=motion_masks,
            predictor=predictor,
            raw_proposals_vis_dir=raw_proposals_vis_dir,
            component_labels_per_frame=component_labels_per_frame,
        )

    def _frames_to_tensors(self, chunk_frames, mask_h: int, mask_w: int) -> torch.Tensor:
        frame_arrays = []
        ...
        return frame_tensors.permute(0, 3, 1, 2)

    def _refine_with_sam_multi(self, frame_tensors, motion_masks, predictor, raw_proposals_vis_dir, component_labels_per_frame=None):
        ...
        result = refine_sam_multi_instance(
            frame_tensors=frame_tensors,
            mask_list=motion_masks,
            predictor=predictor,
            config=cfg,
            raw_proposals_vis_dir=raw_proposals_vis_dir,
            image_predictor_model=self._get_sam2_image_model(),
            component_labels_per_frame=component_labels_per_frame,
        )
        refined_masks = result["proposal_merged_masks"].detach().cpu().numpy()
        instance_masks = result["proposal_instance_masks"].detach().cpu().numpy()
        return refined_masks, instance_masks

    def _refine_with_sam_fbms(self, chunk_frames, motion_masks, predictor=None):
        """Apply SAM2 refinement to masks — FBMS-specific path."""
        assert predictor is not None, "predictor must be provided (build it once outside and pass it in)."
        frame_tensors = self._frames_to_tensors(chunk_frames, motion_masks.shape[1], motion_masks.shape[2])
        selected_mode = "multi"
        if self.refine_mode in {"single", "multi"}:
            selected_mode = self.refine_mode
        print(f"  Refine mode selected (FBMS): {selected_mode}")
        ...
        return self._refine_with_sam_multi_fbms(
            frame_tensors=frame_tensors,
            motion_masks=motion_masks,
            predictor=predictor,
        )

    def _refine_with_sam_multi_fbms(self, frame_tensors, motion_masks, predictor):
        cfg = MultiInstanceRefineConfigFBMS(
            prior_threshold=0.5,
            max_proposals_per_frame=8,
            max_tracks=16,
            min_track_length=2,
        ...
        result = refine_sam_multi_instance_fbms(
            frame_tensors=frame_tensors,
            mask_list=motion_masks,
            predictor=predictor,
            config=cfg,
        )
        refined_masks = result["merged_masks"].detach().cpu().numpy()
        instance_masks = result["instance_masks"].detach().cpu().numpy()
        return refined_masks, instance_masks

    def _refine_with_sam_single(self, frame_tensors, motion_masks, predictor):
        ...
        # original_prior = (motion_masks > 0.5).astype(np.float32)
        # final_masks = np.logical_or(refined_masks_np > 0.5, original_prior > 0.5).astype(np.float32)
        instance_masks = refined_masks_np[:, None, :, :]
        return refined_masks_np, instance_masks

    def _select_refine_mode(self, motion_masks):
        if self.refine_mode in {"single", "multi"}:
            return self.refine_mode, None

        dataset_default_mode = self._get_dataset_default_refine_mode()
        if dataset_default_mode is not None:
            return dataset_default_mode, None

        multi_candidate_frames = 0
        valid_frames = 0
        component_labels_by_frame = []
        for frame_idx, mask in enumerate(motion_masks):
            component_count, second_ratio, labels = self._mask_component_stats(
                mask,
                threshold=0.5,
                return_labels=True,
            )
            component_labels_by_frame.append(labels)
            if component_count == 0:
                continue
            valid_frames += 1
            if component_count >= 2 and second_ratio > 0.13:
    ...
    def _get_dataset_default_refine_mode(self) -> Literal["single", "multi"] | None:
        normalized_name = _normalize_dataset_name(self.current_dataset)
        dataset_mode_map = {
            "2016": "single",
            "davis16": "single",
            "davis16m": "single",
        ...
        return dataset_mode_map.get(normalized_name)

    # def _mask_component_stats(self, mask, threshold: float = 0.9):
    ...
    #     second_ratio = component_areas[1] / float(total_area) if len(component_areas) > 1 else 0.0
    #     return len(component_areas), second_ratio
    ...
    def _mask_component_stats(
        ...
        if vis_dir is not None:
            ...
            name = f"{frame_idx:05d}" if frame_idx is not None else "debug"

            # 1. 保存原始 mask 热力图
            raw = mask_np.astype(np.float32)
            raw = raw - raw.min()
            raw = raw / (raw.max() + 1e-6)
            raw_u8 = (raw * 255).astype(np.uint8)
            raw_color = cv2.applyColorMap(raw_u8, cv2.COLORMAP_JET)
            cv2.imwrite(os.path.join(vis_dir, f"{name}_raw_mask.png"), raw_color)

            # 2. 保存二值 mask
            binary_vis = (binary_mask * 255).astype(np.uint8)
            cv2.imwrite(os.path.join(vis_dir, f"{name}_binary_thr{threshold:.2f}.png"), binary_vis)
        if total_area == 0:
            if return_labels:
                return 0, 0.0, None
            return 0, 0.0
        ...
        for label_idx in range(1, num_labels):
            ...
            if area >= min_component_area:
                valid_label_ids.append(label_idx)
                valid_areas.append(area)

        paired = sorted(zip(valid_label_ids, valid_areas), key=lambda x: x[1], reverse=True)
        component_areas = [area for _, area in paired]
        ...
        second_ratio = (
            component_areas[1] / float(component_areas[0])
            if len(component_areas) > 1 and component_areas[0] > 0
            else 0.0
        )

        filtered_labels = None
        if return_labels:
            valid_set = set(label_id for label_id, _ in paired)
            filtered_labels = np.zeros_like(labels)
            for label_id in valid_set:
                filtered_labels[labels == label_id] = label_id

        if vis_dir is not None:
            name = f"{frame_idx:05d}" if frame_idx is not None else "debug"

            # 3. 保存连通域彩色图
            label_vis = np.zeros((*binary_mask.shape, 3), dtype=np.uint8)
            ...
            for label_idx in range(1, num_labels):
                label_vis[labels == label_idx] = colors[label_idx]

            cv2.imwrite(os.path.join(vis_dir, f"{name}_components.png"), label_vis)

            # 4. 保存面积信息
            with open(os.path.join(vis_dir, f"{name}_stats.txt"), "w") as f:
                f.write(f"threshold: {threshold}\n")
                f.write(f"total_area: {total_area}\n")
                f.write(f"num_components: {len(component_areas)}\n")
                f.write(f"component_areas: {component_areas[:30]}\n")
                f.write(f"largest_area: {component_areas[0]}\n")
                if len(component_areas) > 1:
                    f.write(f"second_area: {component_areas[1]}\n")
                else:
                    f.write("second_area: 0\n")
                f.write(f"second_ratio: {second_ratio:.6f}\n")

        if return_labels:
            return len(component_areas), second_ratio, filtered_labels
        return len(component_areas), second_ratio

    def _get_sam2_image_model(self):
        if self._sam2_image_model is not None:
            return self._sam2_image_model

        model_config = self.sam2_config_path or os.environ.get("SAM2_CONFIG_PATH", DEFAULT_SAM2_CONFIG_PATH)
        checkpoint = self.sam2_checkpoint_path or os.environ.get("SAM2_CHECKPOINT_PATH", DEFAULT_SAM2_CHECKPOINT_PATH)
        self._sam2_image_model = build_sam.build_sam2(
            model_config,
            checkpoint,
            device="cuda",
        )
    ...
    def _save_davis_predictions(self, all_motion_masks, output_dir, seq_name, 
        ...
        # id_pred_dir = None
        id_overlay_dir = None
        if all_instance_masks is not None:
            # id_pred_dir = os.path.join(output_dir, 'id_predictions')
            id_overlay_dir = os.path.join(output_dir, 'id_visualizations')
            # os.makedirs(id_pred_dir, exist_ok=True)
            os.makedirs(id_overlay_dir, exist_ok=True)
        ...
        for i, (mask, original_size) in enumerate(zip(all_motion_masks, original_frame_sizes)):
            ...
            if save_overlay and frames_dir is not None and all_frame_files is not None:
                if i < len(all_frame_files):
                    ...
                    if all_instance_masks is not None and i < len(all_instance_masks):
                        id_mask = self._create_id_mask(
                            all_instance_masks[i],
                            original_size,
                        )
                        # id_mask.save(os.path.join(id_pred_dir, f'{i:05d}.png'))
                        id_overlay = self._create_id_overlay(original_frame, id_mask)
                        id_overlay.save(os.path.join(id_overlay_dir, f'{i:05d}.png'))
        
        print(f"  Saved {len(all_motion_masks)} masks to {pred_dir}")
        if save_overlay:
            print(f"  Saved {len(all_motion_masks)} overlays to {overlay_dir}")
        if all_instance_masks is not None:
            # print(f"  Saved {len(all_motion_masks)} id masks to {id_pred_dir}")
            print(f"  Saved {len(all_motion_masks)} id overlays to {id_overlay_dir}")

    def _create_id_mask(self, instance_masks, original_size):
        """Create a per-pixel instance-id mask from [K, H, W] masks."""
        original_width, original_height = original_size
        instance_masks = np.asarray(instance_masks)
        if instance_masks.ndim != 3:
            raise ValueError(f"instance_masks must have shape [K, H, W], got {instance_masks.shape}")

        id_mask = np.zeros((original_height, original_width), dtype=np.uint16)
        for inst_idx in range(instance_masks.shape[0]):
            mask = instance_masks[inst_idx].astype(np.float32)
            ...
            if binary_mask.any():
                id_mask[binary_mask] = inst_idx + 1

        return Image.fromarray(id_mask, mode='I;16')

    def _create_id_overlay(self, image, id_mask, alpha=0.55):
        """Create colored overlay from a uint16 instance-id mask."""
        if image.size != id_mask.size:
            id_mask = id_mask.resize(image.size, Image.NEAREST)

        img_array = np.array(image)
        id_array = np.array(id_mask)
        overlay_array = img_array.astype(np.float32).copy()

        for inst_id in np.unique(id_array):
            if inst_id == 0:
                continue
            color = np.array(_INSTANCE_COLORS[(int(inst_id) - 1) % len(_INSTANCE_COLORS)], dtype=np.float32)
            mask = id_array == inst_id
            overlay_array[mask] = (
                overlay_array[mask] * (1.0 - alpha) + color * alpha
            )
    ...
    def _compute_dataset_metrics(self):
        ...
        dataset_metrics = {
            ...
            'JR': np.mean([seq_metrics['JR'] for seq_metrics in self.all_sequence_metrics if seq_metrics is not None]),
...
def main():
    ...
    parser.add_argument("--refine_mode", type=str, default="auto", choices=["single", "multi", "auto"],
                        help="SAM refine mode selection strategy")
    parser.add_argument("--davis", type=str, default="2017-M",
                        help="Dataset name, e.g., '2017-M', 'davis17-m', '2016', 'davis16', 'fbms59', 'segtrack'")

    args = parser.parse_args()
    args.davis = _canonical_dataset_name(args.davis)
